			...
			switch(this->action){
				...
				case 3:
					if(this->IsValidRegister(this->match[2]) && this->IsValidRegister(this->match[3])){
						this->microprogramming.WriteRegister(this->match[2], this->microprogramming.GetRegister(this->match[3]));
					}
					else if(this->IsValidMMAccess(this->match[2]) && this->IsValidRegister(this->match[3])){
						string address = this->match[2];
						address = address.substr(1, address.size() - 2);
						this->microprogramming.MemoryWrite(address, this->microprogramming.GetRegister(this->match[3]));
					}
					else if(this->IsValidRegister(this->match[2]) && this->IsValidMMAccess(this->match[3])){
						string address = this->match[3];
						address = address.substr(1, address.size() - 2);
						string data = this->microprogramming.MemoryRead(address);
						this->microprogramming.WriteRegister(this->match[2], data);
					}
					else{
